...
/// <c>state.db</c>.</para>
///
/// <para><b>Ablage und Netz entstehen getrennt und auf Abruf.</b> <c>windows</c> braucht keine
/// Datenbank, <c>token status</c> keine Warteschlange. Würde alles im Konstruktor gebaut, legte
/// ein reiner Anzeigebefehl <c>state.db</c> an — und bei eingeschalteter Schwärzung obendrein
/// die Schlüsseldatei, deren Anlegen ausdrücklich erst beim ersten Bedarf geschehen soll.</para>
/// </remarks>
...
public sealed class Composition : IDisposable
{
    ...
    /// </param>
    public Composition(AppConfig config, ILoggerFactory loggers, string? statePath = null)
    {
...
internal sealed class FixedTokenStore : ITokenStore
{
    ...
    /// 403 abgewiesen, ununterscheidbar von einem abgelaufenen Token.
    /// <para>Wortgleich zu <c>TanssLogWatcher.App/Runtime/RuntimeComposition.cs</c>: Wer hier
    /// etwas ändert, ändert es dort mit.</para>
    /// </remarks>
